...
describe('DataConverterBoxSource', () => {
  ...
  describe('generateBoxes', () => {
    it('should convert JSON to YAML, TOML, and XML when requested', async () => {
      const boxes = await DataConverterBoxSource.generateBoxes(jsonInput, { yaml: true, toml: true, xml: true, json: true });
...
age: 30`;
      const boxes = await DataConverterBoxSource.generateBoxes(yamlInput, { json: true, toml: true, xml: true });
...
age = 30`;
      const boxes = await DataConverterBoxSource.generateBoxes(tomlInput, { json: true, yaml: true, xml: true });
    ...
    it('should convert XML to JSON, YAML, and TOML when requested', async () => {
      ...
      const boxes = await DataConverterBoxSource.generateBoxes(xmlInput, { json: true, yaml: true, toml: true });
    ...
    it('should only show requested format when options are provided', async () => {
      const boxes = await DataConverterBoxSource.generateBoxes(jsonInput, { yaml: true });
      const names = boxes.map(b => b.props.name);
      expect(names).toContain('YAML Output');
    ...
    it('should return empty array for invalid input', async () => {
      const boxes = await DataConverterBoxSource.generateBoxes('just a random string', {});
      expect(boxes).toHaveLength(0);
    });
  });
});
