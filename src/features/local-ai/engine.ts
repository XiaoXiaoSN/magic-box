...
// No React or browser globals: the worker owns one engine, and tests can inject
// a deterministic GPU/runtime rather than downloading a model in CI.
export class LocalAIEngine {
  ...
  async handle(command: AICommand): Promise<void> {
    // Cancellation must not wait behind the in-flight generate promise.
    ...
    if (this.active) return; // main-thread client rejects duplicate submissions
    ...
    try {
      if (command.type === 'inspect') {
        ...
        const files = await runtime.ModelRegistry.get_pipeline_files(
          'text-generation', MODEL.id, MODEL_OPTIONS,
        );
        ...
        if (!metadata.length || metadata.some((file) =>
          !file.exists || file.size === null || !Number.isSafeInteger(file.size) || file.size < 0,
        )) throw new LocalAIError('metadata');
        const bytes = metadata.reduce((total, file) => total + (file.size ?? 0), 0);
        if (!Number.isSafeInteger(bytes) || bytes <= 0) throw new LocalAIError('metadata');
        const cached = await runtime.ModelRegistry.is_pipeline_cached(
          'text-generation', MODEL.id, MODEL_OPTIONS,
        );
        this.send({ type: 'available', id: command.id, info: { bytes, cached } });
      ...
      // Raw model exceptions may contain user content. Only send allowlisted
      // error codes to the UI, never console.log or telemetry payloads.
      const code = error instanceof LocalAIError ? error.code :
        error instanceof Error && error.name === 'QuotaExceededError' ? 'storage' :
          command.type === 'generate' ? 'generation' :
            command.type === 'inspect' ? 'metadata' : 'load';
  ...
  private getSession(id: number): Promise<Session> {
    this.sessionPromise ??= (async () => {
      ...
      const model = await runtime.AutoModelForCausalLM.from_pretrained(MODEL.id, {
        ...MODEL_OPTIONS,
        progress_callback: (event: RuntimeProgress) => {
          ...
          this.send({ type: 'progress', id, file: event.file ?? '', progress });
        },
      });
      try {
        // An adapter is not proof that this model can execute. A tiny warm-up
        // catches unsupported operators / allocation failures before "ready".
