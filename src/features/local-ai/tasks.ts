...
const instructions = {
  ...
  translate:
    'Translate the supplied text faithfully. Output only the translation.',
  rewrite:
    'Rewrite the supplied text clearly and naturally without changing its meaning. Output only the rewrite.',
  summarize:
    'Summarize only the supplied text. Do not add facts that are not in the text.',
} as const;
...
export function checkTokenBudget(tokens: number): void {
  // Counts the rendered chat template, including system and special tokens.
